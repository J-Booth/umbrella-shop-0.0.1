...
class BillingPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        name_vcmd = (self.register(self.confirm), '%P', '%S', 'name')
        self.name = ttk.Entry(self, validate="key",
        ...
        self.phone = ttk.Entry(self, validate="key",
        ...
        city_vcmd = (self.register(self.confirm), '%P', '%S', 'city')
        self.city = ttk.Entry(self, validate="key",
                              validatecommand=city_vcmd)
        self.city.grid(row=14, column=8, columnspan=7, sticky="e")

        # Postcode label and entry
...
-----------------------------------
    ...
    def confirm(self, P, S, field_type):
        """
        Validation for the name, phone, address, city and postcode entries.
        ...
        allowed_value = P   # So the values of P and S are understandable
        inserted_value = S  # But are set as param so that %P and %S are used.
        if ((field_type == 'postcode' or field_type == 'phone') and
                len(allowed_value) == 0):
                inserted_value = 0
                allowed_value = "0"
        elif ((field_type == 'name' or field_type == 'city') and
                len(allowed_value) == 0):
                inserted_value = ""
                allowed_value = "0"
        else:
        ...
        if (field_type == 'phone' and isinstance(inserted_value, int) and
                0 < len(allowed_value) <= 10):
            return True
        elif (field_type == 'name' and not isinstance(inserted_value, int) and
                0 < len(allowed_value) <= 20):
            return True
        elif (field_type == 'city' and not isinstance(inserted_value, int) and
                0 < len(allowed_value) <= 20):
            return True
        elif (field_type == 'postcode' and isinstance(inserted_value, int) and
                1 <= len(allowed_value) <= 4):
